...
TRACE_EVENT(ems_find_busy_cpus,
	...
	TP_PROTO(struct task_struct *p, int busy_thr, int nr_running, int num_idle_cpu),

	TP_ARGS(p, busy_thr, nr_running, num_idle_cpu),
	...
	TP_fast_assign(
		...
		__entry->num_idle_cpu		= num_idle_cpu;
	),

	TP_printk("comm=%s pid=%d busy_thr=%d nr_running=%d num_idle_cpu=%d",
		__entry->comm, __entry->pid, __entry->busy_thr,
		__entry->nr_running, __entry->num_idle_cpu)
);
...
TRACE_EVENT(ems_take_util_snapshot,
	...
	TP_printk("comm=%s pid=%d cpu=%u task_util=%lu task_util_clamped=%lu "
		  "util=%lu util_wo=%lu util_with=%lu "
		  "rt_util=%lu dl_util=%lu nr_running=%u idle=%d",
		  __entry->comm, __entry->pid, __entry->cpu,
		  __entry->task_util, __entry->task_util_clamped, __entry->util,
		  __entry->util_wo, __entry->util_with, __entry->rt_util,
		  __entry->dl_util, __entry->nr_running, __entry->idle)
);
