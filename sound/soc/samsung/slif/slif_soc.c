...
static int slif_soc_clk_enable(struct slif_data *data)
{
	struct device *dev = data->dev;
	int ret = 0;
	...
	if (data->clk_mux_dmic_aud0) {
		...
		slif_info(dev, "clk_mux_dmic_aud0 aft:%d\n",
	...
	if (data->clk_mux_dmic_aud1) {
		...
		slif_info(dev, "clk_mux_dmic_aud1 aft:%d\n",
...
static void slif_soc_clk_disable(struct slif_data *data)
{
	if (!data->clk_enable)
		return;
...
#error "SLIF_SOC_VERSION is not defined"
	...
	ret = slif_soc_clk_enable(data);
	if (ret < 0)
		slif_err(dev, "Failed to enable clocks: %d\n", ret);
