...
static void prefer_cpu_get(struct tp_env *env, struct cpumask *mask)
{
	...
	/* If task util is under 128, apply small task mask */
	if (env->task_util < (SCHED_CAPACITY_SCALE >> 3))
		cpumask_and(mask, mask, &prefer_cpu.small_task_mask);
}
				...
				unsigned long val, void *v)
{
	...
	cpumask_copy(&prefer_cpu.small_task_mask,
...
{
	int cpu, max_spare_cpu, min_energy_cpu = -1;
	unsigned long max_spare_cap, spare_cap[VENDOR_NR_CPUS] = {0,};
	unsigned long min_energy = ULONG_MAX;
	struct cpumask candidates;

	cpumask_clear(&candidates);

	/* get max spare capacity */
	max_spare_cpu = find_max_spare_cpu(env, false, spare_cap);
	max_spare_cap = spare_cap[max_spare_cpu];

	for_each_cpu(cpu, &env->fit_cpus) {
		int shift;

		/*
		 * Based on the maximum spare capacity cpu, the cpu
		 * having similiar level of spare capacity becomes
		 * candidate. Criteria of similiar level is different
		 * whether cpu is idle or not. A cpu with a spare
		 * capacity greater than 75% of maximum spare capacity
		 * becomes candidate, and if cpu is idle, the percentage
		 * goes down to 50% to increase probability of being
		 * selected.
		 */
		if (env->cpu_stat[cpu].idle)
			shift = 1;
		...
		if (spare_cap[cpu] >= (max_spare_cap -
					(max_spare_cap >> shift)))
			cpumask_set_cpu(cpu, &candidates);
	}

	/* find min energy cpu among candidate */
	for_each_cpu(cpu, &candidates) {
		unsigned long next_cap, energy;

		/* Get next capacity of cpu in coregroup with task */
		next_cap = cpufreq_get_next_cap(env, &pcpu_csd(cpu)->cpus, cpu, false);
	...
	trace_ems_find_semi_perf_cpu(env, min_energy_cpu,
				max_spare_cpu, &candidates);
...
{
	int cpu, nr_running = 0, busy_thr, num_idle_cpu = 0;

	/*
	 * HACK: The need for busy cpus is being considered. If busy cpus is
	 * unnecessary, this function will be deleted, otherwise, this HACK
	...
	/*
	 * Currently, "busy cpus" is applied to only slowest cpus.
	 * If necessary, it can be expanded to faster cpus.
	 *
	 * If following conditions are met, it is considered as busy cpus.
	 * Busy cpus will be excluded from fit cpus.
	 *
	 *  1. sum of nr_running > busy threshold(= num cpu * 1.5 + 1)
	 *  2. all cpus are running
	 */
	busy_thr = cpumask_weight(cpu_slowest_mask());
	busy_thr = (busy_thr * 3 / 2) + 1;
	...
	if (nr_running > busy_thr && !num_idle_cpu) {
		cpumask_copy(mask, cpu_slowest_mask());
		trace_ems_find_busy_cpus(env->p, busy_thr,
...
static int find_fit_cpus(struct tp_env *env)
{
	...
	/* Except busy cpus from fit_cpus if all fit_cpus are not busy */
	if (!cpumask_subset(&env->fit_cpus, &mask[4]))
