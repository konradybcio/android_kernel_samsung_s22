...
static int (*ptr_get_next_cap)(struct tp_env *env, struct cpumask *cpus, int dst_cpu, bool clamp);

/* register call back when cpufreq governor initialized */
void cpufreq_register_get_next_cap(int (*fn)(struct tp_env *env, struct cpumask *cpus, int dst_cpu, bool clamp))
{
	ptr_get_next_cap = fn;
}

/* unregister call back when cpufreq governor initialized */
void cpufreq_unregister_get_next_cap(int (*fn)(struct tp_env *env, struct cpumask *cpus, int dst_cpu, bool clamp))
{
	if (ptr_get_next_cap == fn)
		ptr_get_next_cap = NULL;
}
...
{
	if (likely(ptr_get_next_cap)) {
		int cap = ptr_get_next_cap(env, cpus, dst_cpu, clamp);
		if (cap > 0)
			return cap;
	}
