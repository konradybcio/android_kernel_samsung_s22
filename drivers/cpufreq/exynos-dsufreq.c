...
static int scale_dsufreq(unsigned int target_freq, unsigned int relation)
{
	...
	dbg_snapshot_freq(dsufreq.dss_type, dsufreq.cur_freq, resolve_freq, DSS_FLAG_IN);

	ret = cal_dfs_set_rate(dsufreq.cal_id, resolve_freq);
	if (ret)
		goto out;
...
out:
	dbg_snapshot_freq(dsufreq.dss_type, dsufreq.cur_freq, resolve_freq,
						...
						unsigned int relation)
{
	...
	ret = scale_dsufreq(target_freq, relation);
