...
static struct hdr_context *hdr_acquire_context(struct exynos_hdr *hdr)
{
	...
	for (i = 0; i < MAX_HDR_CONTEXT; i++) {
		ctx = &hdr->ctx[i];
		if (atomic_cmpxchg(&ctx->used, 0, 1) == 0)
			break;
	}

	if (i == MAX_HDR_CONTEXT) {
		hdr_debug(hdr, "all ctx pools are occupied\n");
		return NULL;
	}
				...
				struct hdr_context *ctx)
{
	...
	if (ctx) {
		if (atomic_cmpxchg(&ctx->used, 1, 0) == 0)
