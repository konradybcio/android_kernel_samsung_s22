...
static void exynos_panel_get_max_fps(struct device *dev, struct exynos_panel *ctx)
{
	struct drm_device *drm_dev = NULL;
	struct drm_crtc *crtc;
	struct exynos_drm_crtc *exynos_crtc = NULL;
	...
	if (!(ctx->desc->modes[0].exynos_mode.mode_flags & MIPI_DSI_MODE_VIDEO))
		return;	// @command mode
...
static void exynos_panel_set_dqe_xml(struct device *dev, struct exynos_panel *ctx)
{
	...
	ret = of_property_read_string(np, "dqe-suffix", &xml_suffix);
	if (ret != 0 || xml_suffix == NULL)
		return;
