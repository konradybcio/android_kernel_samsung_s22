...
#define DQE_CREATE_SYSFS_FUNC(name) \
	...
	{ \
		int ret = 0; \
		struct exynos_dqe *dqe = dev_get_drvdata(dev); \
		if (count <= 0 || buffer == NULL || dqe_lut == NULL) \
			return -1; \
		mutex_lock(&dqe->lock); \
		dqe_debug(dqe, "%s +\n", __func__); \
...
static ssize_t dqe_xml_show(struct exynos_dqe *dqe,
			struct dqe_ctx_int *lut, char *buf)
{
	struct dsim_device *dsim = decon_get_dsim(dqe->decon);

	if (strlen(dqe->xml_suffix) > 0) {
		sprintf(dqe_str_result, "%s.xml", dqe->xml_suffix);
	} else {
		if (!dsim || !dsim->dsi_device || !dsim->dsi_device->dev.driver)
			return 0;

		if (strlen(dsim->dsi_device->name) > 0)
			sprintf(dqe_str_result, "%s.xml", dsim->dsi_device->name);
		else
			sprintf(dqe_str_result, "%s.xml", dsim->dsi_device->dev.driver->name);
	}
