	...
	u32 *addr, u32 num_buffers)
{
	...
	/* reset fro frame counter */
	if (num_buffers == 1)
		CSIS_DMAX_CHX_SET_R(base, CSIS_DMAX_CHX_R_FRO_FRM, 0);
	...
	CSIS_DMAX_CHX_SET_R(base, CSIS_DMAX_CHX_R_FCNTSEQ, (1 << num_buffers) - 1);
	CSIS_DMAX_SET_R(base, CSIS_DMAX_R_FRO_INT_FRAME_NUM, (num_buffers - 1));

	for (i = 0; i < num_buffers; i++)
		CSIS_DMAX_CHX_SET_R(base, CSIS_DMAX_CHX_R_ADDR1 + i, *(addr + i));

	val = 0;
	val = CSIS_DMAX_CHX_SET_V(val, CSIS_DMAX_CHX_F_UPDT_FRAMEPTR, (num_buffers - 1));
